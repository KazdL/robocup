...
void MotionController::PostUpdate(const UpdateInfo &_info, const EntityComponentManager &_ecm)
{
  ...
  // gzmsg<<"see_ball:"<<this->dataPtr->param_server_node->parameters->stp.see_ball<<std::endl;
...
void MotionController::MotionControllerPrivate::motion_fuse()
{
  ...
  temp_motions.clear();
  planner_lock.unlock();
  this->planner_ready = true;// modified, to let planner work
}
...
void MotionController::MotionControllerPrivate::store_plan()
{
  ...
  for(int i = 0; i < len; ++i)
  {
    ...
    if(fabs(new_gait.x) > 0.001 || fabs(new_gait.t) > 0.0001)    temp_motions.push_back(new_gait);
  }
}
