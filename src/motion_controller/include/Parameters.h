...
namespace dmotion
{
    ...
    struct OneFootLandingParam //用来取代原OneFootLanding.h中宏定义的参数
    {
        ...
        double UPBODY_CONSTANT_PITCH = 0.2;
        double ANKLE_OFFSET_X = 1;
        double ANKLE_OFFSET_Y = 0;
        double ANKLE_OFFSET_Z = 10;
    ...
    struct PendulumWalkParam
    {
        ...
        //表示抬脚高度曲线的三个向量
        std::vector<double> foot_z_t;
        std::vector<double> foot_z_p;
        std::vector<double> foot_z_s;
    ...
    struct KickParam
    {
        ...
        std::string RIGHT_KICK_FILES = "/home/ubuntu/dancer-workspace/workspaces/core/src/dmotion/config/";

        std::vector<double> COM_X;
        std::vector<double> COM_Y;
        std::vector<double> COM_Y_S;
        //std::vector<double> LEFT_COM_Y;
        std::vector<double> COM_Z;
        std::vector<double> ANKLE_X;
        std::vector<double> ANKLE_Y;
        //std::vector<double> LEFT_ANKLE_Y;
    ...
    std::string label = "walk";  //用于输出是什么阶段的步态     
    };

    struct robot_state
    {
        std::vector<double> com = {0,0,0};  //质心x,y,yaw
        std::vector<double> dcom = {0,0,0}; //质心 xdot, ydot, yawdot
        std::vector<double> foot = {0,0,0}; //摆动腿 x,y,yaw
        std::vector<double> dfoot = {0,0,0}; //摆动腿 xdot, ydot, yawdot    
        std::vector<double> rpy = {0,0,0}; // 当前质心rpy
        int index; //当前在一个步行周期内的第几个点，用于计算剩余时间
    };

    struct StatusParam
    {
        std::vector<gait_element> gait_queue;
        gait_element last_gait;
        gait_element tmp_gait;  //方便随处使用暂存的临时单个步态对象，注意每次对其改变都要保证其每个成员的最新
        robot_state cur_state;
        bool adjust_foothold = 0;
        ...
        std::vector<double> action_global = {0,0,0};
    ...
    //新规则backward
        ...
        int back_climb_speed = 2;
        int forward_climb_speed = 2;
